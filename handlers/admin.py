...
async def edit_menu(callback: CallbackQuery):
    photo = InputMediaPhoto(media=FSInputFile('img/banner_menu.png'), parse_mode='html', caption=f'Выберите нужную вам категорию:⬇️')
...
async def add_category(callback: CallbackQuery, state: FSMContext):
    photo = InputMediaPhoto(media=FSInputFile('img/banner_menu.png'), parse_mode='html', caption='Введите наименование категории:⬇️')
    await callback.message.edit_media(media=photo,
...
async def open_catalog(callback: CallbackQuery):
    ...
    print(id)
    photo = InputMediaPhoto(media=FSInputFile('img/banner_menu.png'), parse_mode='html', caption=f'Вы выбрали категорию {await get_api(f'user/name_category/{id}')}')
    await callback.message.edit_media(media=photo,
...
async def edit_category(callback: CallbackQuery):
    id = callback.data.split('_')[-1]
    item = await rq.get_name_category(id)
    await callback.message.edit_text(f'Что вы хотите сделать с категорией <b>{item}?</b>',
...
async def edit_name_category(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text('Введите новое название категории', reply_markup=kb_admin.cancel_add_product_fsm)
...
async def edit_name(message: Message, state: FSMContext):
    await state.update_data(name=message.text)
    data = await state.get_data()
    await rq.edit_category(data['id'], message.text)
    await state.clear()
    await message.answer(f'Категория была успешно изменена')
    await message.answer(text=f'Вы выбрали категорию {await rq.get_name_category(data["id"])}',
...
async def edit_name_items_(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        text='Введите название товара', reply_markup=kb_admin.cancel_add_product_fsm)
...
async def reg_edit_name_items(message: Message, state: FSMContext):
    await state.update_data(name=message.text)
    data = await state.get_data()
    await rq.edit_items(data['id'], 'name', data['name'])
    await state.clear()
...
async def edit_name_items_(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(id=id)
    await callback.message.answer(
        text='Введите описание товара', reply_markup=kb_admin.cancel_add_product_fsm)
...
async def reg_edit_name_items(message: Message, state: FSMContext):
    await state.update_data(description=message.text)
    data = await state.get_data()
    await rq.edit_items(data['id'], 'description', data['description'])
    await state.clear()
    await message.answer('Товар успешно изменён')
    id = data['id']
    item = await rq.get_item(id)
...
async def edit_name_items_(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(id=id)
    await callback.message.answer(
        text='Введите себестоимость товара', reply_markup=kb_admin.cancel_add_product_fsm)
...
async def reg_edit_name_items(message: Message, state: FSMContext):
    try:
        ...
        data = await state.get_data()
        await rq.edit_items(data['id'], 'cost_price', data['cost_price'])
        await state.clear()
        await message.answer('Товар успешно изменён')
        id = data['id']
        item = await rq.get_item(id)
...
async def edit_name_items_(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(id=id)
    await callback.message.answer(
        text='Введите цену товара', reply_markup=kb_admin.cancel_add_product_fsm)
...
async def reg_edit_name_items(message: Message, state: FSMContext):
    try:
        ...
        data = await state.get_data()
        await rq.edit_items(data['id'], 'price', data['price'])
        await state.clear()
        await message.answer('Товар успешно изменён')
        id = data['id']
        item = await rq.get_item(id)
...
async def edit_name_items_(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(id=id)
    await callback.message.answer(
        text='Введите изображение товара', reply_markup=kb_admin.cancel_add_product_fsm)
...
async def reg_edit_name_items(message: Message, state: FSMContext):
    await state.update_data(img=message.photo[-1].file_id)
    data = await state.get_data()
    await rq.edit_items(data['id'], 'img', data['img'])
    await state.clear()
    await message.answer('Товар успешно изменён')
    id = data['id']
    item = await rq.get_item(id)
...
async def spam(callback: CallbackQuery, state: FSMContext):
    photo = InputMediaPhoto(media=FSInputFile('img/banner_menu.png'), parse_mode='html', caption='Введите текст который вы хотите отправить')
...
async def spam_status(message: Message, state: FSMContext):
    if message.text == '✅':
        data = await state.get_data()
        await state.clear()
        for tg_id in await rq.get_users():
...
async def delete_items(callback: CallbackQuery):
    ...
    await rq.delete_items(item_id)
    await callback.message.answer('Товар успешно удален, для возвращения в главное меню нажмите /root')
    await callback.answer('')
